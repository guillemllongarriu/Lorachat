...
#pragma pack(pop)
  ...
  T* getPayload(packet<T>* packet) {
    if (packet->type == DATA_P)
      return (T*) (((dataPacket<T>*) (&packet->payload))->payload);
   ...
   */
  template <typename T>
  packet<T>* createPacket(uint16_t dst, uint16_t src, uint8_t type, T* payload, uint8_t payloadSize) {
   ...
   */
  template <typename T>
  packet<T>* createPacket(T* payload, uint8_t payloadSize, bool hasVia) {
    int payloadLength = payloadSize * sizeof(T);

    int viaSize = (hasVia ? sizeof(dataPacket<uint32_t>::via) : 0);
   ...
   */
  template <typename T>
  void setPackedForSend(packet<T>* tx, uint8_t priority, uint32_t timeout) {
    packetQueue<uint32_t>* send = createPacketQueue<uint32_t>(tx, priority, timeout);
...
#pragma pack(pop)
  ...
  size_t getPacketLength(packet<T>* p){
    return sizeof(packet<T>) + p->payloadSize + (p->type == DATA_P ? sizeof(dataPacket<uint32_t>::via) : 0);
  }
